...
def search_folder(path):
        ...
        for file in os.listdir(path):
            ...
            with open(filepath, 'rb') as text:
                for line in text.readlines():
                    line_counter += 1
                    if len(line) < len(b"2009-01-01") or line[:len(b"2024-04-")] != b"2024-04-":
    ...
    if dic:
        #key_counter = 0
        #with open(f"{path}_unknown_date.txt", 'wb') as text:
        with open(f"{path}_unknown_date.csv", 'w', newline="") as csvFile:
            ...
            for key in dic:
                ...
                for line in dic[key]:
                    ...
                    # global tracker3
                    # tracker3 = line[1]
                    # print(tracker1, tracker2, tracker3)
                    final_line = line[1].decode("utf-8")
                    for rep in characters:
                        final_line = final_line.replace(rep[0], rep[1])
                    #writer.writerow([key, line[0], line[1].decode("utf-8").replace('\r', '\\r').replace('\n', '\\n').replace('\x00', '\\x00')])
...
def main():
    ...
    #folders = [r"H:\Operator Awareness Tool\Unzipped Files - EC - 2024-04-24 - 2024-04-30"]
